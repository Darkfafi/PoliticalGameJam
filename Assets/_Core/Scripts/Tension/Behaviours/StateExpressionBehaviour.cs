    ...
    private void SetStateAnimations(TensionState tensionState)
    {
        ...
        switch (tensionState)
        {
            case TensionState.Idle:
                this.transform.DOScale(1.1f, 0.85f).SetLoops(-1, LoopType.Yoyo).SetEase(Ease.Linear);
                break;
            case TensionState.Pushy:

                this.transform.DOScale(1.1f, 0.85f).SetEase(Ease.OutElastic).OnComplete(()=> {

                    this.transform.DOScale(1, 0.4f).OnComplete(()=> { SetStateAnimations(TensionState.Pushy); });
                });
                break;
            case TensionState.Aggression:
                this.transform.DOScale(1.15f, 0.35f).SetLoops(-1, LoopType.Yoyo).SetEase(Ease.InBack);
                break;

            case TensionState.Outbreak:
                this.transform.DOScale(1.20f, 0.185f).SetLoops(-1, LoopType.Yoyo).SetEase(Ease.InBack);
                break;
        }
    }
}
